...
def user_panel(request):
    ...
    if request.user.is_teacher:
        ...
        context.update({'users': Paginator(Passwords.objects.filter(teacher=teacher).order_by('pk'), 100).page(page)})
        ...
        context.update({'users': Paginator(Passwords.objects.filter(user__is_teacher=False).order_by('pk'), 100).page(page)})
        if request.method == 'POST':
            with open(settings.BASE_DIR/'media/user.txt', 'w') as f:
