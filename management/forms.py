...
class TeacherCompetitionForm(ModelForm):
    ...
    class Meta:
        ...
        widgets = {'start_time': DateTimeInput(attrs={'type': 'datetime-local', 'class': 'time', 'disabled':'disabled'}, format="%Y-%m-%dT%H:%M"),
                'end_time': DateTimeInput(attrs={'type': 'datetime-local', 'class': 'time', 'disabled':'disabled'}, format="%Y-%m-%dT%H:%M"),
                'is_unlimited': CheckboxInput(attrs={'onchange': 'time_close()', 'id': 'unlim'}),
                'is_visible_result': CheckboxInput(),
                'is_simulator': CheckboxInput(),
                }
